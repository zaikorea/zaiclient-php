...
class RateEvent extends BaseEvent {
     ...
     *     ];
     *     $rate_event = new RateEvent($customer_id, $rate_actions);
     * 
        ...
        foreach ($rate_actions as $rate_action) {
            ...
            if (array_keys($rate_action) != array('item_id', 'value')) {
                throw new \InvalidArgumentException(
                    sprintf(Config::ARR_FORM_ERRMSG, self::class, __FUNCTION__, 2, "[ ['item_id' => P12345, 'value' => 5.0] ] (1D array available if targeting single rate action)")
                );
            }
